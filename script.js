...
function activateCategory(categories) {
  const filters = Array.isArray(categories) ? categories : [categories]; // ✅ 배열 보장

  images.forEach(img => {
    const imgCats = JSON.parse(img.dataset.category || "[]"); // 빈 배열 처리
  ...
  if (likeBtn) likeBtn.classList.toggle('dark-mode', darkModeOn);

  const closeCollection = document.getElementById('closeCollection');
  if (closeCollection) closeCollection.classList.toggle('dark-mode', darkModeOn);
...
// 기존 querySelectorAll(".add-btn") 제거
